...
import { setWrite } from '#utils/state/features/writeSlice';
import * as libs from '../libs/libs';

export const useSendMessage = () => {
  const newMessage = libs.useSelector((state: AppState) => state.write);
  ...
  const sendMessage = () => {
    ...
    const message = { type: 'message', chat_id: chat.data.chatId, 
      message: { message: newMessage.data.text, user_id: user.data.email, chat_id: chat.data.chatId } };
    current.send(JSON.stringify(message));
    dispatch(setWrite(''));
  };
  const setNewMessage = (e: React.ChangeEvent<HTMLTextAreaElement>) => dispatch(setWrite(e.target.value));
